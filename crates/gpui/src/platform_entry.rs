...
pub fn background_executor() -> gpui_platform::BackgroundExecutor {
    current_platform(true).background_executor()
}

pub fn application() -> gpui_runtime::Application {
    ...
    gpui_runtime::Application::with_platform(current_platform(false))
}

pub fn headless() -> gpui_runtime::Application {
    gpui_runtime::Application::with_platform(current_platform(true))
}
...
#[cfg(target_family = "wasm")]
pub fn application_with_web_backend(backend_preference: WebBackendPreference) -> gpui_runtime::Application {
    let platform = Rc::new(gpui_web::WebPlatform::new_with_backend(
        true,
        backend_preference,
    ));
    let http_client = std::sync::Arc::new(platform.fetch_http_client());
    gpui_runtime::Application::with_platform(platform).with_http_client(http_client)
}

/// Unlike `application`, this function returns a single-threaded web application.
#[cfg(target_family = "wasm")]
pub fn single_threaded_web() -> gpui_runtime::Application {
