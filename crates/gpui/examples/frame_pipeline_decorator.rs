...
use gpui::{
    App, Bounds, Context, FocusId, FontWeight, FramePipeline, IntoHsla, PreparedRoots, Render,
    StandardImmediatePipeline, Window, WindowBounds, WindowMetrics, WindowOptions,
...
impl<P: FramePipeline> FramePipeline for ThrottledPipeline<P> {
    ...
    fn evaluate_roots(&mut self, window: &mut Window, cx: &mut App) -> PreparedRoots {
        self.inner.evaluate_roots(window, cx)
    }

    fn layout_roots(&mut self, window: &mut Window, roots: &mut PreparedRoots, cx: &mut App) {
        self.inner.layout_roots(window, roots, cx);
    }

    fn paint_roots(&mut self, window: &mut Window, roots: PreparedRoots, cx: &mut App) {
    ...
    fn end_frame(&mut self, window: &mut Window, cx: &mut App, focus: Option<FocusId>) {
...
fn stat_card(value: String, label: &'static str, color: impl IntoHsla) -> impl IntoElement {
    div()
        .flex()
        .flex_col()
        .gap_1()
