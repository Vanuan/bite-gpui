...
pub(crate) fn derive_action(input: TokenStream) -> TokenStream {
    ...
    */
    ...
    let build_fn_body = if no_json {
        ...
        quote! { Err(gpui_runtime::private::anyhow::anyhow!(#error_msg)) }
    } else if is_unit_struct {
        quote! { Ok(Box::new(Self)) }
    } else {
        quote! { Ok(Box::new(gpui_runtime::private::serde_json::from_value::<Self>(_value)?)) }
    };

    let json_schema_fn_body = if no_json || is_unit_struct {
        quote! { None }
    } else {
        quote! { Some(<Self as gpui_runtime::private::schemars::JsonSchema>::json_schema(_generator)) }
    };
    ...
    TokenStream::from(quote! {
        ...
        impl gpui_runtime::Action for #struct_name {
            ...
            fn partial_eq(&self, action: &dyn gpui_runtime::Action) -> bool {
                action
                    .as_any()
                    .downcast_ref::<Self>()
            ...
            fn build(_value: gpui_runtime::private::serde_json::Value) -> gpui::Result<Box<dyn gpui_runtime::Action>> {
                #build_fn_body
            }

            fn action_json_schema(
                _generator: &mut gpui_runtime::private::schemars::SchemaGenerator,
            ) -> Option<gpui_runtime::private::schemars::Schema> {
                #json_schema_fn_body
            }
