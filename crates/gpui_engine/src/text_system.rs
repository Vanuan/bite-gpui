...
//!
//! [`PlatformTextSystem`](crate::PlatformTextSystem) is the lower-level shaping SPI;
//! [`TextSystem`] adds the engine's font-resolution, metric, wrapping, and per-frame
//! line-layout caching on top of it. `gpui_engine_default` ships the standard
//! implementation.
...
impl dyn TextSystem {
    ...
    /// implementation when a caller needs implementation-specific layout features
    /// that the shared SPI does not expose.
