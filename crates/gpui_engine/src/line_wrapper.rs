//! The line wrapper, the wrapping vocabulary it consumes, and the pooled handle it is
//! borrowed through.
//!
//! [`LineWrapper`] measures and truncates text through the shared [`TextSystem`] SPI, so
//! it lives down here beside the fragments and boundaries it operates on. The pool that
//! hands wrappers out belongs to the concrete text system in `gpui_engine_default`.
...
impl LineWrapper {
    ...
    /// Sets the letter spacing applied between glyphs when measuring.
    ...
    /// Whether `c` is a word character for wrapping purposes (i.e. not whitespace and
    /// not CJK punctuation or ideographs, which may be broken anywhere).
    ...
    /// The advance width of `text` in this wrapper's font, including letter spacing.
