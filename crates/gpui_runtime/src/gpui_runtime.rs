//! GPUI's process harness: how an application is built, configured and started.
//!
//! [`Application`] is what a host program holds in order to bring GPUI up — the
//! `main` function of a desktop app, or an embedder driving its own run loop.
//! Everything an application *is* once it is running lives in [`gpui_authoring`],
//! which this crate drives rather than contains.
//!
//! The `gpui` crate is a thin facade over both, so applications keep importing
//! `gpui`.

#![warn(missing_docs)]
#![allow(unused_mut)] // The harness keeps the authoring layer's `mut` callback bindings verbatim.

mod application;
mod pipeline;

pub use application::{Application, ApplicationHandle};
pub use pipeline::{FramePipelineExt, InstrumentedPipeline, PhaseMetrics, ThrottledPipeline};
