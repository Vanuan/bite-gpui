...
//!
//! GPUI recognizes gestures from raw `TouchEvent`s in a single, portable arena:
//! recognizers compete for in-flight touches, winners claim them, and losers are
//! cancelled. Recognized gestures are surfaced through *existing* semantic events
//! wherever possible, a tap becomes `ClickEvent::Touch`, a pan becomes
//! `ScrollWheelEvent`s carrying a `TouchPhase`, and a pinch becomes `PinchEvent`s
//! — so components written against `on_click` and scroll containers work
//! untouched on mobile.
...
#[derive(Debug)]
#[doc(hidden)]
pub enum RecognizedTouchGesture {
...
impl TouchGestureRecognizer {
    ...
    #[doc(hidden)]
    pub fn pending_long_press(&self) -> Option<(TouchId, Duration)> {
    ...
    #[doc(hidden)]
    pub fn offer_long_press(&mut self, id: TouchId) -> Option<RecognizedTouchGesture> {
        let TouchGestureState::Pending {
            touch,
            long_press_offered,
            ..
    ...
    #[doc(hidden)]
    pub fn resolve_long_press(&mut self, claimed: bool) {
        if !claimed {
            return;
        }
    ...
    #[doc(hidden)]
    pub fn offer_touch_drag(&mut self, id: TouchId) -> Option<RecognizedTouchGesture> {
        let TouchGestureState::Pending {
            touch,
            touch_drag_offered,
            ..
    ...
    #[doc(hidden)]
    pub fn resolve_touch_drag(&mut self, claimed: bool) {
        if !claimed {
            return;
        }
        let state = mem::replace(&mut self.state, TouchGestureState::Idle);
    ...
    #[doc(hidden)]
    pub fn tick_momentum(&mut self) -> Option<RecognizedTouchGesture> {
