...
// The layout vocabulary now lives in the engine layer, so the engine can name it without
// depending on `taffy`. Re-exported here so `gpui_authoring::style::…` and `gpui::style::…`
// are unchanged.
pub use gpui_engine::{
    AlignContent, AlignItems, AlignSelf, Display, FlexDirection, FlexWrap, GridTemplate,
    GridTemplateMinSize, JustifyContent, JustifyItems, JustifySelf, Overflow, Position,
};
...
// `Filter` and `ScaledFilter` moved down into `gpui_engine`; re-export them so this
// module keeps providing the names it used to define.
pub use gpui_engine::filter::{Filter, ScaledFilter};
...
pub use gpui_types::{StrikethroughStyle, UnderlineStyle};
