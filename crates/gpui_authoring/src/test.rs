...
//!
//! The output of the `gpui_runtime::test` macro is understood by other rust test runners, so you can use it with `cargo test`
...
/// This is intended for use with the `gpui_runtime::test` macro
...
pub fn observe<T: 'static>(entity: &Entity<T>, cx: &mut TestAppContext) -> Observation<()> {
    ...
    let _subscription = cx.update(|cx| {
        cx.observe(entity, move |_, _| {
            let _ = gpui_platform::block_on(tx.send(()));
        })
    });
    let rx = Box::pin(rx);
