...
// The line-wrap vocabulary moved down into `gpui_engine`; re-exported so this module keeps
// providing the names it used to define, while `LineWrapper` (which borrows the text
// system) stays above.
pub use gpui_engine::{Boundary, LineFragment, WrapBoundaryCandidate};
