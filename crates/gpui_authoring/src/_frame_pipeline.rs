...
//! immediate-mode loop: rebuild the view tree, lay it out, paint what it drew.
//!
...
//!
//! Anywhere above this crate. A decorator that times the root passes of the
//! pipeline it wraps and forwards everything else needs nothing but this crate's
//! public surface.
//!
//! ## Composing pipelines
//!
//! [`FramePipeline`](crate::FramePipeline) is object-safe and every pass defaults
//! to the [`Window`](crate::Window) method implementing it, so independent
//! concerns compose as decorators: a pipeline wraps another, changes the one pass
//! it cares about, and forwards the rest.
//!
//! ## Installing one
//!
