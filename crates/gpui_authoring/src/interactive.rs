//! The element-side input vocabulary: the [`InputEvent`] conversion traits, the
//! click event types, and the `Sealed` impls that bind them to the platform
//! payloads in [`gpui_platform`].
use crate::{
    Bounds, FileDropEvent, KeyDownEvent, KeyUpEvent, Modifiers, ModifiersChangedEvent,
    MouseButton, MouseDownEvent, MouseExitEvent, MouseMoveEvent, MousePressureEvent, MouseUpEvent,
    PinchEvent, Pixels, PlatformInput, Point, ScrollWheelEvent, TouchEvent, seal::Sealed,
};
