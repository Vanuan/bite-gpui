...
use crate::{
    App, AsyncWindowContext, Bounds, BoundsExt, ImageSource, Pixels, Point, RenderImage, Size,
    SvgRenderer, Window,
};
...
// `PlatformInputHandler` moved down into `gpui_platform` as a delegate seam; re-exported so
// the `crate::platform::…` path this module used to provide keeps resolving.
pub use gpui_platform::PlatformInputHandler;


// The platform SPI traits and the window-creation vocabulary moved down into `gpui_platform`;
// re-exported so the `crate::platform::…` paths this module used to provide keep resolving.
pub use gpui_platform::{
    Platform, PlatformWindow, ScreenCaptureSource, WindowBounds, WindowKind, WindowOptions,
    WindowParams,
};
// `PlatformTextSystem` and `NoopTextSystem` moved down into `gpui_engine` with the rest of
// the shaping vocabulary; re-exported so this module keeps providing the names it used to
// define.
pub use gpui_engine::{NoopTextSystem, PlatformTextSystem};
...
/// [`Image`]; as a trait, method-call syntax is unchanged wherever
...
mod image_tests {
    ...
    fn test_image_to_image_data_applies_exif_orientation() {
        let image = Image::from_bytes(
            ImageFormat::Jpeg,
            include_bytes!("../../gpui/examples/legacy/image/exif-orientation-rotate-180.jpg")
                .to_vec(),
        );
