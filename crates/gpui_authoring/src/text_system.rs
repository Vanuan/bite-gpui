//! The window-scoped text layout subsystem.
//!
//! [`WindowTextSystem`] converts the authoring layer's [`TextRun`]s into font runs and
//! drives the engine's text system, which owns shaping, metrics, the line-wrapper pool
//! and the per-frame layout caches. The shaping results — [`ShapedLine`] and
//! [`WrappedLine`] — stay here because painting them needs a window.
...
// The text-system, line-layout and line-wrap vocabulary moved down into `gpui_engine`
// and `gpui_engine_default`; re-export it so this module keeps providing the names it
// used to define.
pub use gpui_engine::{
    Boundary, Font, FontFamilyId, FontId, FontMetrics, FontRun, FontStyle, FontWeight,
    LineFragment, LineLayout, LineLayoutIndex, LineWrapper, LineWrapperHandle, RenderGlyphParams,
    SUBPIXEL_VARIANTS_X, SUBPIXEL_VARIANTS_Y, ShapedGlyph, ShapedRun, TextRenderingMode, TextRun,
    TextSystem, TruncateFrom, WrapBoundary, WrapBoundaryCandidate, WrappedLineLayout, font,
    font_name_with_fallbacks, font_name_with_fallbacks_shared,
};
pub use gpui_engine_default::{
    AsCacheKeyRef, CacheKey, CacheKeyRef, DefaultTextSystem, FontIdWithSize, HashedCacheKey,
    HashedCacheKeyRef,
};
