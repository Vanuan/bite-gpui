                    ...
                    measure("frame duration", || {
                        handle
                            .update(&mut cx, |_, window, cx| {
                                ...
                                if force_render || window.should_render_frame() {
...
impl Window {
    ...
    ///
    /// Draws through this window's [`FramePipeline`]: the pipeline's `draw` runs
    /// the frame's phases in order, so installing another pipeline changes how a
    /// frame is built without changing this call site.
    #[profiling::function]
    pub fn draw(&mut self, cx: &mut App) -> ArenaClearNeeded {
        let pipeline = self.frame_pipeline.clone();
        pipeline.borrow_mut().draw(self, cx)
    }
    /// Opens a frame: resets the scratch state a frame rebuilds and takes
    /// ownership of the invalidations this frame owes.
    ///
    /// Runtime API: [driven by a `FramePipeline`][FramePipeline], not by elements.
    pub fn begin_frame(&mut self, cx: &mut App) {
        #[cfg(debug_assertions)]
        self.frame_phase.enter(
    ...
    pub(crate) fn should_render_frame(&self) -> bool {
        ...
        let metrics = WindowMetrics {
            bounds: self.platform_window.bounds(),
            viewport: Bounds::new(Point::default(), self.viewport_size),
            content_size: self.viewport_size,
            scale_factor: self.scale_factor,
            display_id: self.display_id,
            is_active: self.active.get(),
            is_fullscreen: self.platform_window.is_fullscreen(),
        };
        self.frame_pipeline
            .borrow_mut()
    ...
    ///
    /// The root element is produced here; a view's [`Render`] is not called
    /// until [`layout_roots`](Self::layout_roots) lays it out, because an
    /// element builds its tree when it is asked for a layout.
    pub fn evaluate_roots(&mut self, cx: &mut App) -> PreparedRoots {
        #[cfg(debug_assertions)]
        self.frame_phase
            .enter(&[FramePhase::Begun], FramePhase::Evaluated, "evaluate_roots");
        let root = self.root.as_ref().unwrap().clone().into_any_element();

        // A prompt displaces the other overlays rather than stacking with them,
        // so at most one of prompt, drag and tooltip is drawn.
        let mut prompt = None;
        let mut drag = None;
        if let Some(handle) = self.prompt.take() {
    ...
    pub fn layout_roots(&mut self, roots: &mut PreparedRoots, cx: &mut App) {
        #[cfg(debug_assertions)]
        self.frame_phase
            .enter(&[FramePhase::Evaluated], FramePhase::LaidOut, "layout_roots");
    ...
    pub fn paint_roots(&mut self, mut roots: PreparedRoots, cx: &mut App) {
        #[cfg(debug_assertions)]
        self.frame_phase
            .enter(&[FramePhase::LaidOut], FramePhase::Painted, "paint_roots");
...
// `WindowId` is now defined in the `gpui_platform` leaf and re-exported through `gpui`.
