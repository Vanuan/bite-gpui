//! Executor types shared by `gpui` and its platform backends.
//!
//! The `TaskExt` extension trait stays in `gpui`, because it adds an `&App`-taking method
//! to the leaf's `Task`.

use crate::{PlatformDispatcher, PlatformScheduler};
