//! Profiler hooks the executor needs.
//!
//! The journal itself lives in `gpui`; this module holds the one piece the executor
//! runtime and the platform scheduler share with it, so it cannot stay above the leaf.
...
/// the journal decrements it when that runnable is polled, so the journal can
