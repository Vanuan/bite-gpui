...
mod tests {
    ...
    fn auto_sized_axes_snap_padding_proportionally() {
        ...
        // Both axes auto-sized: each pair snaps as a whole.
        let snapped = padding_to_taffy(&padding, Pixels(16.0), 1.0, true, true);
        let expected_left = 7.0 * (2.5 / 7.5);
        assert_eq!(snapped.left, LengthPercentage::length(expected_left));
        assert_eq!(snapped.right, LengthPercentage::length(7.0 - expected_left));
        assert_eq!(snapped.top, LengthPercentage::length(6.5));
        assert_eq!(snapped.bottom, LengthPercentage::length(6.5));

        // A definite width snaps each edge on its own.
        let snapped = padding_to_taffy(&padding, Pixels(16.0), 1.0, false, true);
        assert_eq!(snapped.left, LengthPercentage::length(2.0));
        assert_eq!(snapped.right, LengthPercentage::length(5.0));
        assert_eq!(snapped.top, LengthPercentage::length(6.5));
        assert_eq!(snapped.bottom, LengthPercentage::length(6.5));

        // An auto axis at a fractional scale factor.
        let padding = Edges {
            top: Pixels(6.75).into(),
            right: Pixels(0.0).into(),
            bottom: Pixels(6.75).into(),
            left: Pixels(0.0).into(),
        };
        let snapped = padding_to_taffy(&padding, Pixels(16.0), 2.0, true, true);
        assert_eq!(snapped.top, LengthPercentage::length(13.5));
        assert_eq!(snapped.bottom, LengthPercentage::length(13.5));
    }
