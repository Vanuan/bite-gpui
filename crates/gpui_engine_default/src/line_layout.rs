//! The per-frame line-layout cache and the keys that address it.
//!
//! The cache shapes lines through a [`PlatformTextSystem`] and keeps the previous frame's
//! layouts so the text system can re-adopt them. [`crate::DefaultTextSystem`] owns one.

use collections::FxHashMap;
use gpui_engine::{
    FontRun, LineLayout, LineLayoutIndex, LineWrapper, PlatformTextSystem, WrapBoundary,
    WrappedLineLayout,
};
...
/// A per-frame cache of shaped and wrapped line layouts, retaining the previous frame so
/// reused layouts survive a frame boundary.
...
/// The boundaries at which `layout`'s text should wrap to `wrap_width`, at most
/// `max_lines` lines deep.
