...
fn test_chunk_highlights_follow_edits_and_theme_changes(cx: &mut TestAppContext) {
    ...
    let theme_without_keyword =
        SyntaxTheme::new([("function".to_string(), gpui_types::rgba(0x0000ffff).into())]);
...
fn test_language_change_invalidates_cached_chunk_highlights(cx: &mut TestAppContext) {
    ...
    let theme = SyntaxTheme::new([
        ("keyword".to_string(), gpui_types::rgba(0xff0000ff).into()),
        ("function".to_string(), gpui_types::rgba(0x00ff00ff).into()),
        ("variable".to_string(), gpui_types::rgba(0x0000ffff).into()),
    ]);
